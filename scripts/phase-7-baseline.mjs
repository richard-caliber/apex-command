#!/usr/bin/env node
// Apex Phase 7 sprint — baseline KV snapshot taken at P7.0.
// Rollback safety net: if anything in P7.1-P7.6 goes catastrophically wrong, restore from this.
// Reads .env.local for KV_REST_API_URL and KV_REST_API_TOKEN.
// Writes: data/phase-7-baseline-{ISO-date}.json
...
// Post-Magnificent canonical key set. M4 deleted 13 legacy keys; this list reflects
// what should still exist after the sprint close. Snapshot anything that's present
// for safety.
const KNOWN_KEYS = [
...
async function main() {
  ...
  const snapshot = {
    ...
    sprint: "apex-phase-7-operationalise",
    phase: "P7.0",
    kv_url: KV_URL,
    keys: {},
  };
  ...
  const snapPath = join(ROOT, "data", `phase-7-baseline-${today}.json`);
